...
pub struct RtcpRrReportBlockPacket<'p> {
    packet: ::pnet_macros_support::packet::PacketData<'p>,
}
#[derive(PartialEq)]
/// A structure enabling manipulation of on the wire packets
pub struct MutableRtcpRrReportBlockPacket<'p> {
    packet: ::pnet_macros_support::packet::MutPacketData<'p>,
}
impl<'a> RtcpRrReportBlockPacket<'a> {
    /// Constructs a new RtcpRrReportBlockPacket. If the provided buffer is less than the minimum required
    /// packet size, this will return None.
    #[inline]
    pub fn new<'p>(packet: &'p [u8]) -> Option<RtcpRrReportBlockPacket<'p>> {
        if packet.len() >= RtcpRrReportBlockPacket::minimum_packet_size() {
            use pnet_macros_support::packet::PacketData;
            Some(RtcpRrReportBlockPacket {
    ...
    /// Constructs a new RtcpRrReportBlockPacket. If the provided buffer is less than the minimum required
    /// packet size, this will return None. With this constructor the RtcpRrReportBlockPacket will
    /// own its own data and the underlying buffer will be dropped when the RtcpRrReportBlockPacket is.
    pub fn owned(packet: Vec<u8>) -> Option<RtcpRrReportBlockPacket<'static>> {
        if packet.len() >= RtcpRrReportBlockPacket::minimum_packet_size() {
            use pnet_macros_support::packet::PacketData;
            Some(RtcpRrReportBlockPacket {
    ...
    /// Maps from a RtcpRrReportBlockPacket to a RtcpRrReportBlockPacket
    #[inline]
    pub fn to_immutable<'p>(&'p self) -> RtcpRrReportBlockPacket<'p> {
        use pnet_macros_support::packet::PacketData;
        RtcpRrReportBlockPacket {
            packet: PacketData::Borrowed(self.packet.as_slice()),
        }
    }
    /// Maps from a RtcpRrReportBlockPacket to a RtcpRrReportBlockPacket while consuming the source
    #[inline]
    pub fn consume_to_immutable(self) -> RtcpRrReportBlockPacket<'a> {
        RtcpRrReportBlockPacket {
    ...
    /// The size (in bytes) of a RtcpRrReportBlock instance when converted into
    /// a byte-array
    #[inline]
    pub fn packet_size(_packet: &RtcpRrReportBlock) -> usize {
...
impl<'a> MutableRtcpRrReportBlockPacket<'a> {
    /// Constructs a new MutableRtcpRrReportBlockPacket. If the provided buffer is less than the minimum required
    /// packet size, this will return None.
    #[inline]
    pub fn new<'p>(packet: &'p mut [u8]) -> Option<MutableRtcpRrReportBlockPacket<'p>> {
        if packet.len() >= MutableRtcpRrReportBlockPacket::minimum_packet_size() {
            use pnet_macros_support::packet::MutPacketData;
            Some(MutableRtcpRrReportBlockPacket {
    ...
    /// Constructs a new MutableRtcpRrReportBlockPacket. If the provided buffer is less than the minimum required
    /// packet size, this will return None. With this constructor the MutableRtcpRrReportBlockPacket will
    /// own its own data and the underlying buffer will be dropped when the MutableRtcpRrReportBlockPacket is.
    pub fn owned(packet: Vec<u8>) -> Option<MutableRtcpRrReportBlockPacket<'static>> {
        if packet.len() >= MutableRtcpRrReportBlockPacket::minimum_packet_size() {
            use pnet_macros_support::packet::MutPacketData;
            Some(MutableRtcpRrReportBlockPacket {
    ...
    /// Maps from a MutableRtcpRrReportBlockPacket to a RtcpRrReportBlockPacket
    #[inline]
    pub fn to_immutable<'p>(&'p self) -> RtcpRrReportBlockPacket<'p> {
        use pnet_macros_support::packet::PacketData;
        RtcpRrReportBlockPacket {
    ...
    /// Maps from a MutableRtcpRrReportBlockPacket to a RtcpRrReportBlockPacket while consuming the source
    #[inline]
    pub fn consume_to_immutable(self) -> RtcpRrReportBlockPacket<'a> {
        RtcpRrReportBlockPacket {
            packet: self.packet.to_immutable(),
    ...
    /// Populates a RtcpRrReportBlockPacket using a RtcpRrReportBlock structure
    #[inline]
    #[cfg_attr(feature = "clippy", allow(used_underscore_binding))]
    pub fn populate(&mut self, packet: &RtcpRrReportBlock) {
        let _self = self;
...
impl<'a> ::pnet_macros_support::packet::PacketSize for RtcpRrReportBlockPacket<'a> {
    #[cfg_attr(feature = "clippy", allow(used_underscore_binding))]
...
impl<'a> ::pnet_macros_support::packet::PacketSize for MutableRtcpRrReportBlockPacket<'a> {
    #[cfg_attr(feature = "clippy", allow(used_underscore_binding))]
    fn packet_size(&self) -> usize {
        let _self = self;
        24
...
impl<'a> ::pnet_macros_support::packet::MutablePacket for MutableRtcpRrReportBlockPacket<'a> {
    #[inline]
...
impl<'a> ::pnet_macros_support::packet::Packet for MutableRtcpRrReportBlockPacket<'a> {
    #[inline]
...
impl<'a> ::pnet_macros_support::packet::Packet for RtcpRrReportBlockPacket<'a> {
    #[inline]
    fn packet<'p>(&'p self) -> &'p [u8] {
        &self.packet[..]
    }
...
/// Used to iterate over a slice of `RtcpRrReportBlockPacket`s
pub struct RtcpRrReportBlockIterable<'a> {
    buf: &'a [u8],
}
impl<'a> Iterator for RtcpRrReportBlockIterable<'a> {
    type Item = RtcpRrReportBlockPacket<'a>;
    fn next(&mut self) -> Option<RtcpRrReportBlockPacket<'a>> {
        use pnet_macros_support::packet::PacketSize;
        use std::cmp::min;
        if self.buf.len() > 0 {
            if let Some(ret) = RtcpRrReportBlockPacket::new(self.buf) {
...
impl<'p> ::pnet_macros_support::packet::FromPacket for RtcpRrReportBlockPacket<'p> {
    type T = RtcpRrReportBlock;
    #[inline]
    fn from_packet(&self) -> RtcpRrReportBlock {
...
impl<'p> ::pnet_macros_support::packet::FromPacket for MutableRtcpRrReportBlockPacket<'p> {
    type T = RtcpRrReportBlock;
    #[inline]
    fn from_packet(&self) -> RtcpRrReportBlock {
        use pnet_macros_support::packet::Packet;
...
impl<'p> ::std::fmt::Debug for RtcpRrReportBlockPacket<'p> {
    #[cfg_attr(feature = "clippy", allow(used_underscore_binding))]
    fn fmt(&self, fmt: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        let _self = self;
        write!(fmt ,
               "RtcpRrReportBlockPacket {{ ssrc : {:?}, fraction_lost : {:?}, cummulative_number_of_packets_lost : {:?}, extended_highest_sequence_number_received : {:?}, interarrival_jitter : {:?}, lsr : {:?}, dlsr : {:?},  }}"
...
impl<'p> ::std::fmt::Debug for MutableRtcpRrReportBlockPacket<'p> {
    #[cfg_attr(feature = "clippy", allow(used_underscore_binding))]
    fn fmt(&self, fmt: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        let _self = self;
        write!(fmt ,
               "MutableRtcpRrReportBlockPacket {{ ssrc : {:?}, fraction_lost : {:?}, cummulative_number_of_packets_lost : {:?}, extended_highest_sequence_number_received : {:?}, interarrival_jitter : {:?}, lsr : {:?}, dlsr : {:?},  }}"
               , _self . get_ssrc (  ) , _self . get_fraction_lost (  ) ,
               _self . get_cummulative_number_of_packets_lost (  ) , _self .
               get_extended_highest_sequence_number_received (  ) , _self .
               get_interarrival_jitter (  ) , _self . get_lsr (  ) , _self .
